...
For Guarded NetKAT, the possible outputs \(O\) contains acceptance \(\acc{}\) and \(\setc{σ}\), where \(σ: X ↛ V\) is a partial function that assign \(σ(x) ∈ V\) to some variable \(x ∈ X\), when \(x\) is in the domain of \(σ\).
...
\begin{mathpar}
  \inferrule
  {\\}
  {(σ, \comAssert{b})\transOut{b[σ]}{\acc{}}}
  \and  
  \inferrule
  {\\}
  {(σ, x := v)\transOut{\true}{\setc{σ[x ↦ v]}}}
  \and  
  \inferrule
  {\\}
  ...
  \and  
  \inferrule
  {(σ, e) \transOut{b}{\acc{}} \\ (σ, f) \transOut{c}{o} \\ o ∈ O}
  {(σ, e; f) \transOut{b ∧ c}{o}}
  \and  
  \inferrule
  {(σ, e) \transOut{b}{\acc{}} \\ (σ, f) \transvia{b ∣ p} (σ', f')}
  {(σ, e; f) \transvia{b ∧ c ∣ p} (σ, f')}
  \and  
  \inferrule
  {(σ, e) \transOut{b}{\setc{σ'}} \\ (σ', f) \transOut{c}{o} \\ o ∈ O}
  {(σ, e; f) \transOut{b ∧ c}{o}}
  \and  
  \inferrule
  {(σ₁, e) \transOut{b}{\setc{σ₂}} \\ (σ₂, f) \transvia{b ∣ p} (σ₃, f')}
  {(σ₁, e; f) \transvia{b ∧ c ∣ p} (σ₃, f')}
  \and  
  \inferrule
  {(σ, e) \transvia{b ∣ p} (σ', e')}
  ...
  \inferrule
  {\\}
  {(σ, e^{(b)}) \transOut{¬b[σ]}{\acc{}}}
  \and  
  \inferrule
  {(σ₁, e) \transOut{c}{\setc{σ₂}} \\  (σ₂, e^{(b)}) \transvia{a ∣ p} (σ₃, f)}
  {(σ₁, e^{(b)}) \transvia{c ∧ a ∣ p} (σ₃, f)}
  \and  
  \inferrule
  {(σ, e) \transvia{c ∣ p} (σ', e')}
  {(σ, e^{(b)}) \transvia{b ∧ c ∣ p} (σ', e'; e^{(b)})}
\end{mathpar}
