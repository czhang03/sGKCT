...
This surprising fact lead to the development of Guarded Kleene Algebra with Tests (GKAT)~\cite{smolka_GuardedKleeneAlgebra_2020}, a (co)algebraic system that seeks to characterize this efficient fragment of KAT.
...
\[
    ...
    \end{tikzcd}
\]
We sometimes use \({∼} ⊆ S × U\) to specify that \(∼\) is a bisimulation between \(S\) and \(U\).
...
This gives us more tool in proving semantic equivalence between two states in GKAT coalgebras: proving bisimulation of these two states in the \emph{non-normalized} GKAT coalgebra can also obtain finite-trance equivalence for the two states.
...
To aid the design of an algorithm based on greedy bisimulation, we turn the coalgebra into a set of concrete criteria, that can be checked by an algorithm.
...
Besides the construction, a classical optimization of the bisimulation-construction algorithm is to look for a equivalence relation instead of a relation~\cite{smolka_GuardedKleeneAlgebra_2020,hopcroft_LinearAlgorithmTesting_1971}.
...
Finally, we present our equivalence-checking algorithm as~\Cref{alg:bisim} which consists of recursively checking the conditions in~\Cref{thm:recursive-construction} with minor optimizations. In the algorithm, ``implies'', ``iff'', ``and'' are logical operators that take booleans (i.e. true or false) and return a boolean. Additionally, \(\&\!\&\) is the logical conjunction operation. It is the same function as the the written ``and''. We use \(\&\!\&\), instead of ``and'', to separate the conditions for clarity; logically \(\&\!\&\) is the same function as ``and'', but syntactically binds less tightly.

Intuitively, the algorithm returns true if and only if a greedy bisimulation \(≈\) can be constructed s.t. \(s ≈ u\).
Note the existence of such \(≈\) is not only equivalent to the trace equivalence of \(s\) and \(u\) (\Cref{thm:greedy-bisim-correctness}), but also equivalent to \(s ≊ u\), where \({≊} ⊆ S × U\) is the maximal bisimulation.
...
First, optimization is to terminate and return false as soon as we know one of \(s, u\) is dead and the other is not.
This is because in order to have \(s ≈ u\) for any greedy bisimulation \(≈\), \(s\) and \(u\) must have the same liveness in their respective GKAT coalgebra (\Cref{thm:greedy-bisim-perserve-liveness}).
In the case where both \(s\) and \(u\) are dead in their respective coalgebra, we will immediate return true, because any dead states pair are necessarily in \(≊\) (\Cref{thm:bisim-between-dead,thm:max-bisim-is-max-greedy-bisim}).
Then, we use a union-find data structure to organize the explored states into equivalence classes, which is justified by~\Cref{thm:greedy-bisim-iff-greedy-bisim-equiv}.
By looking for a greedy bisimulation that is also an equivalence relation, we can use more efficient structures like union-find, instead of set of pairs; the correctness of this optimization is justified by.
...
Although both algorithms have similar worst case complexity, the on-the-fly algorithm will always out-perform the original algorithm in non-worst case situations. This is because the on-the-fly algorithm only invokes liveness checking when necessary. In the extreme case when the two input states are infinite-trace equivalent, the on-the-fly algorithm can even skip liveness checking entirely.
...
\end{mathpar}
This coalgebra is required to satisfy the disjointedness condition, i.e. for all states \(s ∈ S\), the boolean expressions that \(s\) accepts \(ϵ̂(s)\) and the boolean expressions that enables \(s\) to transition \(\{b ∣ ∃ (b, s', p) ∈ δ(s)\}\) are disjoint; also the conjunction of any two distinct expressions from the set \(ϵ̂(s) ∪ \{b ∣ ∃ (b, s', p) ∈ δ(s)\}\) is equivalent to \(0\) under boolean algebra.
...
Similar to the non-symbolic version of the algorithm, \Cref{alg:symb-bisim} first checks if either of the input states is dead, then  recursively checks all the conditions in~\Cref{thm:symb-recursive-construction}, where \(\mathrel{\&\!\&}\) denotes the logical conjunction operator on booleans.
...
Then the correctness of the non-symbolic construction can be used to show the correctness of the symbolic construction i.e. we will prove that the lowering as shown in construction~\labelcref{cons:lowering} will yield the conventional derivative.
...
Basically, a transition is in \(D̂\) if and only if it is derivable by the rules in~\Cref{fig:derivatives-rules}.
...
\end{align*}
Besides guarding transition and acceptance with different conditions in if-statements and while-loops, the guard operator can also be used to simulate uniform continuation. Specifically, we can use \(⟨ϵ̂(s)|δ(s')\) to connect all the accepting transition of \(s\) to the dynamic \(δ(s')\).
...
However, as explained in~\Cref{sec:optimization-implementation}, our implementation uses start dynamics instead of start state to avoid unnecessary lookups.
...
\Cref{thm:hom-thompson-derivative} has several consequences, one of the more obvious ones is that we can use the functoriality of the lowering operation to show the semantic equivalence of the start state in the Thompson's construction and the expression in derivative. 
