...
\begin{theoremEnd}{lemma}\label{thm:dead-construction-greedy-bisim}
    For any eager bisimulation \({≈} ⊆ S × U\) with the underlying bisimulation \({∼}\), the following relation \({∼_d} ⊆ S × U\):
...
This early termination is sound, because in order for \(s ≈ u\) for any greedy bisimulation, \(s\) and \(u\) need to have the same liveness (\Cref{thm:greedy-bisim-perserve-liveness}).
In the case where both \(s\) and \(u\) are dead in their respective coalgebra, we will immediately return true, because all dead-state pairs are in \(≊\) (\Cref{thm:bisim-between-dead,thm:max-bisim-is-max-greedy-bisim}).
