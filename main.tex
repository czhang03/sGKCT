...
\end{mathpar}
This coalgebra is required to satisfy the \emph{disjointedness condition}, i.e. for all states \(s ∈ S\), the boolean expressions that \(s\) accepts \(ϵ̂(s)\) and the boolean expressions that enables \(s\) to transition \(\{b ∣ ∃ (b, s', p) ∈ δ(s)\}\) are disjoint; i.e. the conjunction of any two distinct expressions from the set \(ϵ̂(s) ∪ \{b ∣ ∃ (b, s', p) ∈ δ(s)\}\) is equivalent to \(0\) under Boolean Algebra.
Like the non-symbolic case, we use \(s \transAcc{Ŝ}{b}\) to indicate \(b ∈ ϵ̂(s)\) and \(s \transvia{b ∣ p}_{Ŝ} s'\) to indicate \((b, p, s') ∈ δ̂(s)\).
