...
\begin{mathpar}
    ...
    δ̂: S → 𝒫(\BExp_T × S × K).
\end{mathpar}
This coalgebra is also required to satisfy the disjointedness condition, i.e. for all states \(s ∈ S\), the conjunction of any two expressions from the set \(ϵ̂(s) ∪ \{b ∣ ∃ (b, s', p) ∈ δ(s)\}\) is equivalent to \(0\) under boolean algebra.
...
Similar to the non-symbolic version of the algorithm, we will first check if either of the input states is dead, then  recursively check all the conditions in~\cref{thm:symb-recursive-construction}.
And the symbolic version of the liveness checking algorithm is also based on a graph search algorithm on all the reachable state, where we try to identify an accepting state by checking whether \(⋁ ϵ̂(s) ≡ 0\).
...
The symbolic trace-equivalence algorithm is presented as~\cref{alg:symb-bisim}, where \(\mathrel{\&\!\&}\) is the logical-and operator on the language level, specifying that all the conditions in~\cref{thm:symb-recursive-construction} must be satisfied for the algorithm to return true. 
