...
\begin{definition}[normalizing bisimulation]\label{def:norm-bisim}
    Given two GKAT coalgebra \(S\) and \(U\), and a bisimulation \({∼} ⊆ \norm(S) × \norm(U)\), a normalizing bisimulation \({≈} ⊆ S × U\) is a GKAT coalgebra that contains \(∼\) with the following transition function:
    \[
...
\begin{lemma}
    Given a bisimulation \({∼} ⊆ \norm(S) × \norm(U)\), the following relation \({∼_d} ⊆ S × U\):
    \[{∼_d} ≜ {∼} ∪ \{(s, u) ∣ \text{\(s\) and \(u\) are dead in \(S\) and \(U\)}\},\]
    form a GKAT coalgebra the transition function \(δ_≈\) in~\Cref{def:norm-bisim}
\end{lemma}

\begin{proof}
    We will need to show \(∼_d\) is closed under the transition of \(δ_{≈}\) in~\Cref{def:norm-bisim}.
...
\begin{lemma}\label{thm:norm-bisim-exists-unique}
    For any \(∼\) a normalizing bisimulation with \(∼\) as underlying bisimulation exists and is unique.
\end{lemma}

\begin{proof}
    TODO: sub-coalgebra form a complete lattice.
\end{proof}
...
\begin{lemma}\label{thm:live-in-component-then-live-in-norm-bisim}
    Given a normalizing bisimulation \(≈\), if a state \((s, u)\) is live in \(≈\), then \(s\) and \(u\) are live in \(S\) and \(U\) respectively, which also implies that \((s, u)\) is live in \(∼\).
\end{lemma}

\begin{proof}
    TODO: by preservation of accepting state and liveness chain.
\end{proof}

\begin{lemma}
    Every state in \((s, u) ∈ {≈} ⊆ S × U\) but not in \({∼} ⊆ \norm(S) × \norm(U)\) then \((s, u)\) is dead in \(≈\).
\end{lemma}

\begin{proof}
    By~\Cref{thm:norm-bisim-not-bisim-then-component-dead}, \(s\) is dead in \(S\), then by contrapositive of~\Cref{thm:live-in-component-then-live-in-norm-bisim}, we obtain that \((s, u)\) is dead in \(≈\).
\end{proof}

\begin{theorem}[Correctness]
    Given two states in GKAT coalgebras \(s ∈ S\) and \(u ∈ ,U\), there exists a normalizing bisimulation \({≈} ⊆ S × U\) s.t. \(s ≈ u\), if and only if \(⟦s⟧_S = ⟦u⟧_U\).
\end{theorem}
...
\begin{proof}
    ...
    Because~\Cref{thm:norm-bisim-exists-unique}, therefore there exists a normalizing bisimulation \(≈\) with \(∼\) as its underlying bisimulation. 
    Because the carriers \({∼} ⊆ {≈}\), therefore \(s ∼ u\) implies \(s ≈ u\).
\end{proof}

\begin{theorem}[Recursive Construction]
    Given a relation \({≈} ⊆ S × U\), \(≈\) is a carrier of some normalizing bisimulation if and only if all the following condition are satisfied: for all pair of states \(s ≈ u\),
    \begin{enumerate}
...
\begin{proof}
    To show the \(⟹\) direction, assuming there exists a normalizing bisimulation \(≈\), then it would satisfy all the conditions. 
    ...
    The condition~\labelcref{itm:transition-bisim} holds because when \(s \transvia{α ∣ p}_S s'\) and \(u \transvia{α ∣ p}_U u'\), then \((s, u) \transvia{α ∣ p}_{≈} (s', u')\) by definition of normalizing bisimulation (\Cref{def:norm-bisim}).
    ...
    \begin{itemize}
        ...
        if \(s', u'\) are not both dead, then the condition of \({∼} ⊆ \norm(S) × \norm(U)\) cannot be satisfied, hence \((s', u') ∉ {∼}\), which by~\Cref{thm:norm-bisim-not-bisim-then-component-dead} means that \(s'\) and \(u'\) are both dead, contradicting the premise.
    ...
    TODO: by constructing a \(≈\) and show that \(\norm(≈)\) is a bisimulation. Maybe using the maximal bisimulation?
\end{proof}

\begin{lemma}\label{thm:bisim-between-dead}
    Given any two dead states \(s ∈ S\) and \(u ∈ U\), and the maximal bisimulation \({≡} ⊆ \norm(S) × \norm(U)\), then we have \(s ≡ u\).
\end{lemma}

\begin{theorem}[Equivalence Relation]
    Given two states in GKAT coalgebra \(s ∈ S\) and \(u ∈ U\), there exists a normalizing bisimulation \(≈\) between \(S\) and \(U\) s.t. \(s ≈ u\), if and only if there exists a normalizing bisimulation that is an \emph{equivalence relation} \(≊\) in \(S + U\) s.t. \(s ≊ u\).
\end{theorem}

\begin{proof}
    First, recall that by~\Cref{thm:} \(\norm(≈)\) is a bisimulation between \(\norm(S)\) and \(\norm(U)\).
    ...
    The \(⟹\) direction. 
    Because sub-coalgebra preserves bisimulation (\Cref{thm:sub-coalg-preserve-bisim}), then \(\norm(≈)\) is a bisimulation between \(\norm(S + U)\) and itself.
    We consider \(≡\) to be the maximal bisimulation between \(\norm(S + U)\) and itself, then \(≡\) is an equivalence relation~\cite[]{}.

    Let \(≊\) be the normalizing bisimulation generated by \(≡\), then \(≊\) has the same carrier as \(≡\).
    ...
    Finally, by definition of normalizing bisimulation (\Cref{def:norm-bisim}), \({≡} ⊆ {≊}\).
    Therefore, \(≡\) and \(≊\) share the same carrier and \(≊\) is a normalizing bisimulation that is also an equivalence relation, which also contains \((s, u)\):
    \[(s, u) ∈ {≈} = \norm(≈) ⊆ {≡} ⊆ {≊},\]
    ...
    \begin{itemize}
        \item If \(s ≃ u\), then we can reflect \(≃\)  through \(\norm(S) ⊑ \norm(S + U)\) and \(\norm(U) ⊑ \norm(S + U)\) (\Cref{thm:sub-coalg-preserve-bisim}) to obtain a bisimulation \(∼ ⊆ \norm(S) × \norm(U)\) and \(s ∼ u\).
        Taking the normalizing bisimulation over \(∼\) will yield a desired normalizing bisimulation \(≈\) s.t. \(s ≈ u\).
        \item If \((s, u)\) are not in \(≃\), then they are both dead in \(S + U\) (\Cref{thm:norm-bisim-not-bisim-then-component-dead}), which implies \(s\) and \(u\) are respectively dead in \(S\) and \(U\) (\Cref{thm:sub-coalg-preserve-liveness}).
        Then we take \(≈\) to be normalizing bisimulation generated form the maximal bisimulation between \(S\) and \(U\), which contains \(s\) and \(u\) (\Cref{thm:bisim-between-dead}).
        \qedhere
    \end{itemize}
\end{proof}
...
% % \begin{definition}[Normalizable Bisimulation]
% %     A relation \({≃} ⊆ S × U\) is a normalizable bisimulation if it has a GKAT coalgebra structure \(δ_≃\), and \(\norm(≃)\) is a bisimulation between \(\norm(S)\) and \(\norm(U)\).
% % \end{definition}

% % \begin{lemma}[Preservation of Liveness]
% %     For any normalizable bisimulation \({≃} ⊆ S × U\) if \(s ≃ u\), then \(s\) is live in \(S\) if and only if \(u\) is live in \(U\).
% % \end{lemma}

% % \begin{proof}
% %     Because \(\norm(≃)\) has two projection homomorphism to \(\norm(S)\) and \(\norm(U)\), then 
% % \end{proof}

% % \begin{theorem}[Correctness]
% %     If there exists a normalizable bisimulation \({≃} ⊆ S × U\) s.t. \(s ∼ u\) for \(s ∈ S\) and \(u ∈ U\), then \(⟦s⟧_S = ⟦u⟧_U\).
% % \end{theorem}

% % \begin{theorem}[Construction]
% %     Given two states \(s ∈ S, u ∈ U\) then there exists a normalizable bisimulation \({≃'} ⊆ S × U\) if and only if there exists a relation \({≃} ⊆ S × U\) that satisfy the following conditions:
% %     \begin{enumerate}
...
% % \begin{proof}
% %     The \(⟸\) direction can be shown that 
% % \end{proof}
...
% % \begin{definition}[normalizing bisimulation]\label{def:norm-bism}
% %     A relation \({≃} ⊆ S × U\) on two GKAT coalgebra \(S\) and \(U\) is a normalizing bisimulation if and only if for any two states \(s ≃ u\), all the following holds:
% %     \begin{enumerate}
% %         \item\label{itm:acc-condition} for all \(α ∈ \At\), \(δ_{S}(s, α) = \accept ⟺ δ_{U}(u, α) = \accept\);
% %         \item\label{itm:rej-or-dead} if \(s\) reject \(α\) but \(u\) transitions to \(u'\), then \(u'\) is dead; similarly when \(u\) rejects \(α\);  
% %         \item\label{itm:transition-bisim} If \(δ_{S}(s, α) = (s', p)\) and \(δ_{U}(u, α) = (u', p)\), then \(s' ≃ u'\).
% %         \item\label{itm:transition-dead} If \(δ_{S}(s, α) = (s', p)\) and \(δ_{U}(u, α) = (u', q)\), s.t. \(p ≠ q\), then both \(s'\) and \(t'\) are dead.
% %     \end{enumerate}
% %     And \(≃\) can be endowed with the following GKAT coalgebra structure:
% %     \[
% %         δ_≃((s, u), α) ≜ \begin{cases}
% %             \accept & \begin{aligned}
% %                 & \text{if } δ_S(s, α) = \accept \\[-3px]
...
% \begin{lemma}[Perservation of Liveness]\label{thm:norm-bisim-perserve-liveness}
%     For any normalizing bisimulation \(≃\) and a pair of states \(s ≃ u\), \((s, u)\) is live in \(≃\) if and only if \(s\) is live in \(S\), and similarly for \(u\) in \(U\). 
%     This implies that if \(s ≃ u\), then \(s\) is live in \(S\) if and only if \(u\) is live in \(U\).
% \end{lemma}

% \begin{proof}
%     First by definition of normalizing bisimulation that a state \(s\) is accepting in \(S\) if and only if \((s, u)\) is accepting in \(≃\).

%     Then, to prove the preservation of liveness, we prove the preservation of live chain, i.e. there exists a chain to accepting state in \(≃\): \((s₀, u₀) \transvia{α₁ ∣ p₁} (s₁, u₁) \transvia{α₂ ∣ p₂} (s₂, u₂) \transvia{α₂ ∣ p₂} ⋯ \transvia{αₙ ∣ pₙ} (sₙ, uₙ)\) if and only if there exists a similar accepting chain in \(S\): \(s₀ \transvia{α₁ ∣ p₁} s₁ \transvia{α₂ ∣ p₂} s₂ \transvia{α₂ ∣ p₂} ⋯ \transvia{αₙ ∣ pₙ} sₙ\).

%     Because every state on an accepting chain is live, all we need to prove is that for two states \(s ≃ u\),
%     \begin{enumerate}
%         \item if \((s', u')\) is a live state in \(≃\) and \((s, u) \transvia{α ∣ p}_≃ (s', u')\), then \(s \transvia{α ∣ p}_S s'\)
%         \item if \(s'\) is a live state in \(S\) and \(s \transvia{α ∣ p} s'\), then \((s, u) \transvia{α ∣ p} (s', u')\) for some \(u'\).
%     \end{enumerate}
%     The first goal can be shown by definition of \(δ_{≃}\). 
%     Because \(s'\) in the second goal is live, therefore only condition~\labelcref{itm:transition-bisim} in~\Cref{def:norm-bism} matches the condition, thus implying \(u \transvia{α ∣ p} u'\) and \((s, u) \transvia{α ∣ p} (s', u')\).
% \end{proof}

% \begin{theorem}
%     Given a normalizing bisimulation \(≃: S × U\), its normalization is indeed a bisimulation between \(\norm(S)\) and \(\norm(U)\)
% \end{theorem}

% \begin{proof}
%     We will need to prove the two projection of \(π₁: {≃} → S\) and \(π₂: {≃} → U\) is a homomorphism, without loss of generality, we will only show that \(π₁\) is a homomorphism.

%     First the accepting case, for all \(s ≃ u\):
%     \begin{align*}
%         δ_{\norm(≃)}((s, u), α) = \accept
%         & ⟹ δ_{≃}((s, u), α) = \accept \\
%         & ⟹ δ_{S}(s, α) = \accept \\ 
...
%     Second the transition case, for \(s ≃ u\), because normalizing bisimulation preserves liveness (\Cref{thm:norm-bisim-perserve-liveness}):
%     \begin{align*}
%         & (s, u) \transvia{α ∣ p}_{\norm(≃)} (s', u') \\*
...
%     Finally, we show the rejection case. \((s, u) \transRej{\norm(≃)}{α}\) can be split into two cases, we will show both cases lead to \(s \transRej{\norm(S)}{α}\).
%     \begin{itemize}
%         \item The first case is that \(δ_{≃}((s, u), α) = \reject\). 
%         By definition, this case can happen in following three scenarios: 
%         \begin{itemize}
%             \item when \(s \transvia{α ∣ p} s'\), \(u \transvia{α ∣ q} u'\), and \(p ≠ q\), which implies \(s'\) is dead by condition~\labelcref{itm:transition-dead};  
%             \item when \(s \transvia{α ∣ p} s'\) and \(δ_U(u, α) = \reject\), which implies \(s'\) is dead by condition~\labelcref{itm:rej-or-dead}
%             \item when \(u \transRej{S}{α}\), then \(u \transRej{\norm(S)}{α}\).
%         \end{itemize}
%         Thus, all scenarios cases lead to \(u \transRej{\norm(S)}{α}\).
%         \item The second case is that \((s, u) \transvia{α ∣ p}_{≃} (s', u')\) but \((s', u')\) is dead.
%         By definition \(s \transvia{α ∣ p}_{S} s'\). 
%         By~\cref{thm:norm-bisim-perserve-liveness} and \((s', u')\) is dead, we conclude that \(s'\) is dead and therefore \(s' \transRej{\norm(S)}{α}\).
%     \end{itemize}
% \end{proof}
...
% \begin{lemma}[bisimulation between dead states]\label{thm:bisim-between-dead}
%     Given two dead states \(s ∈ S\) and \(u ∈ U\) and the maximal bisimulation \(≡\) between \(\norm(S)\) and \(\norm(U)\), then \(s ≡ u\).
% \end{lemma}

% \begin{proof}
%     We can construct the singleton bisimulation \({∼} ⊆ \norm(S) × \norm(U)\):
%     \begin{mathpar}% \begin{lemma}[bisimulation between dead states]\label{thm:bisim-between-dead}
%         %     Given two dead states \(s ∈ S\) and \(u ∈ U\), then the singleton bisimulation \({∼} ⊆ \norm(S) × \norm(U)\):
%         %     \begin{mathpar}
%         %         {∼} ≜ \{(s,u)\} \and 
%         %         δ_{∼}((s,u), α) ≜ \reject
...
%         δ_{∼}((s,u), α) ≜ \reject,
%     \end{mathpar}
%     and verify that it is indeed a bisimulation between \(\norm(S)\) and \(\norm(U)\) by unfolding the definition.
% \end{proof}

% \begin{theorem}
%     For any two GKAT coalgebra \(S\) and \(U\), the maximal bisimulation \(≡\) between \(\norm(S)\) and \(\norm(U)\) satisfy all the conditions in~\Cref{def:norm-bism}, i.e. the relation \(≡\) can be a normalizing bisimulation between \(S\) and \(U\).
% \end{theorem}

% \begin{proof}
%     We only verify that \(≡\) satisfy the conditions in~\cref{def:norm-bism}.

%     The condition~\labelcref{itm:transition-bisim} holds when \(s ≡ u\).
%     We first prove that \(s'\) and \(u'\) has to be both live or both dead: because \(δ_S(s, α) = (s', p)\) and \(δ_U(u, α) = (u', p)\),
%     \begin{align*}
%         s' \text{ is live} 
%         & ⟺ δ_{\norm(S)}(s, α) = (s', p) \\
%         & ⟺ δ_{\norm(U)}(u, α) = (u', p) \\
...
%     \end{align*}
%     \begin{itemize}
%         \item If both \(s'\) and \(u'\) are live in \(S\) and \(U\), then \(s' ≡ u'\) because \(≡\) is a bisimulation.
%         \item If both \(s'\) and \(u'\) are dead in \(S\) and \(U\), then they are also dead in \(\norm(S)\) and \(\norm(U)\) (\cref{thm:norm-perserve-liveness}).
%         Finally, by~\Cref{thm:bisim-between-dead}, \(s' ≡ u'\).
%     \end{itemize}
...
%     Given two states in GKAT coalgebra \(s ∈ S, u ∈ U\), consider the maximal bisimulation \({≡} ⊆ \norm(S) × \norm(U)\), \(s ≡ u\) if and only if there exists a normalized bisimulation \({≃} ⊆ S × U\) s.t. \(s ≃ u\).
% \end{lemma}

% \begin{proof}
%     Recall that a relation \({∼} ⊆ \norm(S) × \norm(U)\) is a bisimulation if and only if it has a normalized GKAT coalgebra structure \(δ_∼\), s.t. for all \(s₁ ∼ u₁\):
%     \begin{itemize}
%         \item for all results 
%         \(r ∈ \{\accept, \reject\}\): if \(δ_∼((s₁, u₁), α) = r\), then \(δ_{\norm(S)}(s₁, α) = r\) and \(δ_{\norm(U)}(u₁, α) = r\);
%         \item if \((s₁, u₁) \transvia{α ∣ p}_{∼} (s₂, u₂)\) then \(s₁ \transvia{α ∣ p}_{\norm(S)} s₂\) and \(u₁ \transvia{α ∣ p}_{\norm(U)} u₂\).
%     \end{itemize}

%     We prove \(⟹\) direction by constructing the following normalized bisimulation from the bisimulation \({≡}\) between \(\norm(S)\) and \(\norm(U)\):
...
%         ⟺{}& δ_U(u, α) = \accept
%     \end{align*}
%     The condition \labelcref{itm:acc-condition} also holds if \(s, u\) are both dead, because both can never accept any atoms.
...
%     \end{align*}
%     \begin{itemize}
%         \item If both \(s'\) and \(u'\) are live, then \(s' ≡ u'\), and \(s' ≃ u'\).
%         \item If both \(s'\) and \(u'\) are dead, \(s' ≃ u'\).
%     \end{itemize}

%     The condition~\labelcref{itm:transition-dead} holds, let \(δ_S(s, α) = (s', p)\) and \(δ_U(u, α) = (u', q)\) and \(p ≠ q\). 
%     If \(s' ≃ u'\), then we can show condition~\labelcref{itm:transition-dead} by case analysis on the liveness of \(s'\) and \(u'\).
%     If \(s', u'\) are not both dead, then the condition of \({≡} ⊆ \norm(S) × \norm(U)\) cannot be satisfied, hence \((s', u') ∉ {≡}\), which means that \(s'\) and \(u'\) are both dead, contradicting the premise.
%     Therefore, \(s'\) and \(u'\) needs to be both dead.

%     Then we show the \(⟸\) direction by 
...
%     Then we show the \(⟸\) direction by showing \(≃\) is a bisimulation between \(\norm(S)\) and \(\norm(U)\).
%     Take any pair of states \(s₁ ≃ u₁\), we show that all the conditions of a bisimulation is satisfied.

%     First, we show for all \(α ∈ \At, δ_{\norm(S)}(s₁, α) = \accept ⟺ δ_{\norm(U)}(u₁, α) = \accept\):
%     \begin{align*}
%         δ_{\norm(S)}(s₁, α) = \accept 
%         & ⟺ δ_S(s₁, α) = \accept \\  
%         & ⟺ δ_U(u₁, α) = \accept \\  
...
%     Then, we show for all \(α ∈ \At, δ_{\norm(S)}(s₁, α) = \reject ⟺ δ_{\norm(U)}(u₁, α) = \reject\).
%     Without loss of generality, we only show the \(⟹\) direction.
%     If \(δ_{\norm(S)}(s₁, α) = \reject\), then either \(δ_S(s₁, α) = \reject\), or \(δ_S(s₁, α) = (s₂, p)\) and \(s₂\) is dead.
%     \begin{itemize}
%         \item We consider the case where \(δ_S(s₁, α) = \reject\).
%         By condition~\labelcref{itm:acc-condition}, \(δ_U(u₁, α)\) cannot equal to accept. 
%         If \(δ_U(u₁, α) = \reject\), then the implication holds. 
%         If \(δ_U(u₁, α) = (u₂, q)\), then by condition~\cref{itm:rej-or-dead}, \(u₂\) is dead, which also makes the implication hold.
%         \item We consider the case where \(δ_S(s₁, α) = (s₂, p)\) and \(s₂\) is dead in \(S\).
%         By condition~\labelcref{itm:acc-condition}, \(δ_U(u₁, α)\) cannot equal to accept.
%         If \(δ_U(u₁, α) = \reject\), then the implication holds.
%         If \(δ_U(u₁, α) = (u₂, q)\) and \(p ≠ q\), then by condition~\labelcref{itm:transition-dead}, both \(u₁, u₂\) are dead, then by~\cref{thm:bisim-between-dead}, \(s₂ ≡ s\). 
%         If \(δ_U(u₁, α) = (u₂, q)\) and \(p = q\), then by condition~\labelcref{itm:transition-bisim}, then \(s₂ ≃ u₂\)
%     \end{itemize}
...
%     \end{itemize}
%     Because \(≃ ⊆ \norm(S) × \norm(U)\) is a bisimulation s.t. \(s ≃ u\), and \(≡\) is the maximal bisimulation between \(\norm(S)\) and \(\norm(U)\), we will obtain our result \(s ≡ u\).
% \end{proof}
...
%         \item if \(δ_{S}(s, α) = (s', p)\) and \(δ_{U}(u, α) = (u', q)\), s.t. \(p ≠ q\), then both \(s'\) and \(u'\) are dead.
%     \end{enumerate}
...
The final optimization that we perform is collapsing the sets of boolean expressions of \(\hat{\epsilon}(s)\) into a single boolean expression. Basically, each \(\hat{\epsilon}(s)\) is represented by the boolean disjunction of its elements. Consider the guard operation defined in \Cref{sec:symb-gkat-construction}.
\begin{align*}
