...
Kleene Algebra with Tests (KAT) is a staple algebra system to reason about program equivalence, and have found numerous applications in numerous fields, including software-defined networks~\cite{anderson_NetKATSemanticFoundations_2014a,moeller_KATchFastSymbolic_2024,smolka_ScalableVerificationProbabilistic_2019,smolka_CantorMeetsScott_2017}, program transformations~\cite{kozen_BohmJacopiniTheorem_2008,angus_KleeneAlgebraTests_2001,kozen_NonlocalFlowControl_2008,grathwohl_KAT_2014,kozen_KleeneAlgebraTests_1996}, and program logics~\cite{antonopoulos_AlgebraAlignmentRelational_2023,zhang_DomainReasoningTopKAT_2024,zhang_IncorrectnessLogicKleene_2022c,kozen_HoareLogicKleene_2000,desharnais_ModalKleeneAlgebra_2004}.
Beyond its massive theoretical achievement, Kleene Algebra with Tests has also inspired numerous practical tools for several domains.
Interestingly, despite the PSPACE-complete complexity of KAT, many tools developed are able to match, or even exceed, the performance of state-of-the-art tools in their respective domains~\cite{moeller_KATchFastSymbolic_2024,smolka_ScalableVerificationProbabilistic_2019}.

This surprising fact lead to the development of Guarded Kleene Algebra with Tests (GKAT)~\cite{smolka_GuardedKleeneAlgebra_2020}, a (co)algebraic system that seeks to understand this efficient fragment of KAT.
Indeed, the determinism of GKAT gives it a different coalgebra structure as KAT, and allows for GKAT expression to directly be converted to a linear-sized coalgebra, as opposed to the exponential sized coalgebra produced KAT, in general~\cite{smolka_GuardedKleeneAlgebra_2020,kozen_CoalgebraicTheoryKleene_2017}.

However, the desirable theoretical properties of GKAT does not currently translate to tangible real-world performance. 
surprisingly, the decision procedures for GKAT can often be slower than that of KAT, primarily due to the following two reasons.
...
Thus, to determine finite-trace equivalence, it is necessary to \emph{normalize} the coalgebra first, requiring to compute the entire coalgebra ahead-of-time; and iterate through its entirety.
Without the need to normalize prior to computing bisimulation, the decision procedure of KAT equivalence can terminate as soon as a counter-example has been found, sometimes leading to better performance than the original GKAT decision algorithm~\cite{smolka_GuardedKleeneAlgebra_2020}.

The lack of symbolic algorithm also makes GKAT a less desirable foundation for performance-critical applications.
Although the generated coalgebra from GKAT expression have linear number of states, the number of transitions is exponential to \emph{all} the primitive tests appeared across the entire expression.
This inefficiency have been noted in several pervious literatures, and poses real limitation on real-world systems~\cite{smolka_GuardedKleeneAlgebra_2020,zhang_CFGKATEfficientValidation_2025}.
On the other hand, several symbolic algorithm for KAT have been proven to be effective for generic equality solving and domain-specific applications~\cite{pous_SymbolicAlgorithmsLanguage_2015,moeller_KATchFastSymbolic_2024}.

In this paper, we present two algorithms that address both shortcomings, and demonstrate how coalgebra theory of GKAT serves as the bedrock of the design and correctness of these algorithms.

The first algorithm supports on-the-fly coalgebra generation by performing bisimulation in a greedy manner.
Specifically, the liveness checking of states only kicks in when bisimulation algorithm encountered a disparity.
We support the correctness of this algorithm by designing a coalgebraic concept which we named \emph{greedy bisimulation}.

The second algorithm builds lifts the first into a symbolic setting, where our development is based on a extension of GKAT coalgebra, where we named \emph{symbolic GKAT coalgebra}.
...
This optimization not only allows more compact coalgebra representation, but also allows us to use arbitrary of-the-shelf SAT solvers to resolve boolean constraint, instead of the binary decision diagram presented by the symbolic algorithm of KAT~\cite{pous_SymbolicAlgorithmsLanguage_2015}. 
Finally, we lifted both the derivative construction~\cite{schmid_GuardedKleeneAlgebra_2021} and Thompson's construction~\cite{smolka_GuardedKleeneAlgebra_2020} to symbolic setting, and proved their correspondence, correctness, and finiteness.

To evaluate the practical impact of our work, we have implemented our algorithm in rust. 
...
proving the theoretical elegance of GKAT indeed leads to significant real-world performance differences.
...
\begin{itemize}
    ...
    We also performs a brief complexity analysis, and argue that our algorithm is almost always more efficient than the original normalize and then bisimulate approach~\cite{smolka_GuardedKleeneAlgebra_2020}.
    \item In~\Cref{sec:symb-gkat-greedy-bisim}, we introduced \emph{symbolic GKAT coalgebra}, and lifted the greedy bisimulation algorithm to the symbolic setting.
    \item In~\Cref{sec:symb-gkat-construction}, we lifts the Thompson's construction and derivative construction to the symbolic setting.
    Then we established their connection via a coalgebra homomorphism, which then allows us to derive the correctness and finiteness of one construction from the other.
    \item Finally, in~\Cref{sec:implementation}, we described our implementation of symbolic derivable, symbolic Thompson's construction, and symbolic greedy bisimulation.
    Then we conduct a performance analysis using different SAT solvers, and compared to previous implementations.
    When using minisat as the SAT solver of choice~\cite{}, our implementation can solve most test cases in less than a second with minimal memory usage, whereas previous implementation~\cite{pous_SymbolicAlgorithmsLanguage_2015} almost always result in a timeout.
\end{itemize}
