...
And in the other case, if both \(s\) and \(u\) are dead in their respective coalgebra, we will skip the check on all the reachable state of \((s, u)\) following \(δ_≈\), because a pair of dead states is necessary in the maximal greedy bisimulation \(≊\) (\Cref{thm:bisim-between-dead,thm:max-bisim-is-max-greedy-bisim}).
