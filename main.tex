...
The finality of the model allows us to define the semantics of each state in a given GKAT coalgebra \(S\), which we denote as \(⟦-⟧^{ω}_{S}: S → \mathscr{G}_ω\). Semantic equivalence \(⟦s⟧^{ω}_{S} = ⟦t⟧^{ω}_{T}\) can be identified by bisimulation~\cite{schmid_GuardedKleeneAlgebra_2021} if and only if there exists a bisimulation \({∼} ⊆ S × T\), s.t. \(s ∼ t\).
...
Because of the functoriality, we can show that if two states are infinite-trace equivalent, then these two states are finite-trace equivalent.
This gives us another tool in proving semantic equivalence between two states in GKAT coalgebras: proving bisimulation between two states in the \emph{non-normalized} GKAT coalgebra proves semantic equivalence for them.
