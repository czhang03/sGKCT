    ...
    \begin{algorithmic}
        ...
        \Return {
            ...
            \begin{aligned}
                ...
                & ∀ (b, s', p) ∈ δ̂_S(s), (c, t', q) ∈ δ̂_T(t), (b ∧ c) ≢ 0 ⟹ 
                \begin{cases}
                    \text{\Call{IsDead$_S$}{$s$}} ∧ \text{\Call{IsDead$_T$}{$t$}} & \text{if \(p ≠ q\)} \\
                    \text{\Call{Union}{$s$, $t$}}; \text{\Call{Equiv}{$s', t'$}} & \text{if \(p = q\)}
                \end{cases} \mathrel{\&\!\&}\\
                & ∀ (b, s', p) ∈ δ̂_S(s), c ∈ ρ̂_T(t), (b ∧ c) ≢ 0 ⟹ \text{\Call{IsDead$_S$}{$s'$}} \mathrel{\&\!\&}\\
                & ∀ b ∈ ρ̂_S(s), (c, t', q) ∈ δ̂_T(t), (b ∧ c) ≢ 0 ⟹ \text{\Call{IsDead$_T$}{$t'$}}
            \end{aligned}\)
        }
        \EndIf
        \EndFunction
...
The \(S₁ + S₂\) is the disjoint union of \(S₁\) and \(S₂\), and for any two transition dynamics \(δ₁(s₁): 𝒫(\BExp × S₁ × K)\) and \(δ₂(s₂): 𝒫(\BExp × S₂ × K)\), then \(δ₁^*(s₁) + δ₂^*(s₂): 𝒫(\BExp × (S₁ + S₂) × K)\) is the bifunctorial lift via \(+\).
