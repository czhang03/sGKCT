...
\begin{align*}
    ...
    e^{(b)} & ≜ (b; e)^*; \overline{b}.
\end{align*}
Then the semantics of each expression \(⟦e⟧\) can be computed by the semantics of Kleene Algebra with tests.

However, the semantics of GKAT can also be obtained coalgebraically, specifically, with GKAT coalgebra~\cite{smolka_GuardedKleeneAlgebra_2020,schmid_GuardedKleeneAlgebra_2021}.
...
Concretely, \(\norm(δ_S) : S → G(S)\) is defined as \(\norm(δ_S)(s, α) ≜ \reject\) when \(δ_S(s, α) = (s', p)\) and \(s'\) is dead in \(S\); and \(\norm(δ_S)(s, α) ≜ δ_S(s, α)\) otherwise.
...
For more details on the finite trace semantics, we refer the reader to the work of \Citeauthor{smolka_GuardedKleeneAlgebra_2020}~\cite{smolka_GuardedKleeneAlgebra_2020}.
