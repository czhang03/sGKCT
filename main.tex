...
% However, when considering designing a practical algorithm, we not just cares about the correctness of the derivative construction, we are also interested in the size of the generated automata. 
% Specifically, we are interested in all the reachable states (or expressions) from any expression \(e\), which is exactly the principle sub-coalgebra \(⟨e⟩_{D̂}\).

% \begin{theorem}[Complexity]
%     The number of state in the principle sub-coalgebra \(⟨e⟩_{D̂}\) is linear to the number of primitive actions in \(e\).
% \end{theorem}

% \begin{proof}
%     To prove this, we prove the following two theorem:
