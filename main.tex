...
\begin{proof}
    ...
    Finally, we demonstrate that \(δ_∼\) is well-defined by leveraging the conditions in~\Cref{thm:inductive-construction}. 
...
\begin{corollary}[Correctness]
    For any two states in two GKAT coalgebra \(s ∈ S, u ∈ U\),the four condition in~\Cref{thm:inductive-construction} is satisfied if and only if \(s\) and \(u\) are finite-trace equivalent
\end{corollary}

\begin{proof}
    By the standard argument with normalization preserves sub-coalgebra (\Cref{thm:norm-sub-coalg}), we can obtain 
    \(\norm(⟨s⟩) ⊑ \norm(S)\) and \(\norm(⟨u⟩) ⊑ \norm(U)\).
    Because sub-coalgebra reflects and preserves bisimulation (\Cref{thm:sub-coalg-preserve-bisim}) with the correctness of bisimulation between normalized coalgebras: let \({∼}\) and \({∼'}\) be bisimulations,
    \begin{align*}
        & ∃~ {∼} ⊆ \norm(⟨s⟩) × \norm(⟨u⟩), s ∼ u \\
        ⟺{}& ∃~ {∼'} ⊆ \norm(S) × \norm(U), s ∼ u \\ 
        ⟺{}& ⟦s⟧_{S} = ⟦u⟧_{U}. \qedhere
